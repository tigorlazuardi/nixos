...
in
{
  config =
    if cfg.enable then
      ...
      {
        # Some direnv that I use uses zellij to run multiple commands at the same time.
        # Zellij is nice because it can split log windows, but the session serialization
        # is annoying because it's stored to disk and have to clean it up on new sessions.
